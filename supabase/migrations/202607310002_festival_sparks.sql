...
create unique index bonus_awards_once_per_person
  ...
  where achievement not in ('helpful-spotlight', 'first-light', 'team-joins-in');
...
create unique index bonus_awards_first_review_per_team
  on public.bonus_awards (event_id, achievement, team_id)
  where achievement in ('first-light', 'team-joins-in');
...
$$;
revoke all on function public.sync_bonus_budget(uuid) from public, anon, authenticated;
...
begin
  ...
  if signal_was_new then
    ...
    if not exists (select 1 from public.signals where event_id = signal_event_id and team_id = signal_team_id and id <> saved_signal.id) and not exists (select 1 from public.bonus_awards where event_id = signal_event_id and achievement = 'first-light' and team_id = signal_team_id) then
